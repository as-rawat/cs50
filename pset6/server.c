...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            line = malloc(sizeof(char) * (needle - haystack + 2 + 1));
            ...
            sp[1]++;
            if(line[sp[1]] != 'H' || line[sp[1] + 1] != 'T' || line[sp[1] + 2] != 'T' || line[sp[1] + 3] != 'P' || line[sp[1] + 4] != '/' || line[sp[1] + 5] != '1' || line[sp[1] + 6] != '.' || line[sp[1] + 7] != '1')
            {
              error(505);
              continue;
            }
            ...
            strcpy(path, root);
            ctr = strlen(path);
            for(i = sp[0] + 1; line[i] != '?' && line[i] != ' '; i++)
            {
              path[ctr] = tolower(line[i]);
              ctr++;
            }
            path[ctr] = '\0';
            ...
            char extension[5];
            int p = 0;
            for(i = dot[0] + 1; line[i] != ' ' && line[i] != '?' && p <= 3; i++)
            {
              extension[p] = tolower(line[i]);
              p++;
            }
            extension[p] = '\0';
            // dynamic content
